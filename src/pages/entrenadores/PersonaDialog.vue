<template>
  ...
  ">
                  ...
                  <div class="row q-col-gutter-md">
                    ...
                    <!-- <q-select v-model="localPersona.tipo_persona" label="Tipo de Persona *" outlined dense
                      option-label="label" option-value="value" emit-value map-options class="col-md-6" :options="[
...
function prepareAndSave() {
  ...
  console.log('PREPARE AND SAVEEEEEEEEEEEEEEEEEEEEEEEEEEEEEE', localPersona.value);
  // Añadir campos adicionales automáticos
  localPersona.value.fecha_registro = new Date().toISOString()
  try {
    const current = JSON.parse(sessionStorage.getItem('user'))
    localPersona.value.usuario_registro = current ? (current.usuario || (current.persona && current.persona.nombres) || '') : ''
  } catch {
    localPersona.value.usuario_registro = ''
  }

  const formData = new FormData()
  // Si se debe crear usuario, generar credenciales
  const nombres = (localPersona.value.nombres || '').trim()
  const primerNombre = nombres.split(' ')[0] || ''
  const ci = (localPersona.value.ci || '').toString()
  const username = `${primerNombre}${ci}`
  const password = `${ci}`
