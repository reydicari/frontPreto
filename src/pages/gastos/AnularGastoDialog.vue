    ...
    <q-card class="anular-dialog">
      <q-card-section class="anular-header">
        <div class="header-title-anular">
          ...
          <div class="text-h6 text-weight-bold">Anular Gasto</div>
        </div>
      </q-card-section>
      ...
      <q-card-section class="anular-body">
        ...
        <div class="gasto-info" v-if="gasto">
          <div class="info-row">
            <span class="info-label">Monto:</span>
            <span class="info-value">Bs {{ parseFloat(gasto.monto).toFixed(2) }}</span>
          </div>
          <div class="info-row" v-if="gasto.categorium">
            <span class="info-label">Categoría:</span>
            <span class="info-value">{{ gasto.categorium.nombre }}</span>
          </div>
          <div class="info-row" v-if="gasto.fecha">
            <span class="info-label">Fecha:</span>
            <span class="info-value">{{ formatDate(gasto.fecha) }}</span>
          </div>
          <div class="info-row" v-if="gasto.observacion">
            <span class="info-label">Observación:</span>
            <span class="info-value">{{ gasto.observacion }}</span>
          </div>
        </div>
...
import { anularGasto } from 'stores/gasto-store'
...
async function confirmarAnular() {
  if (!props.gasto?.id) return

  loading.value = true
  try {
    const res = await anularGasto(props.gasto.id)
    if (res.data.ok) {
      emit('anulado')
      cerrar()
    }
