<template>
  <q-page class="q-pa-md page-container" :class="$q.dark.isActive ? '' : 'bg-grey-4'">
    <q-card class="torneo-card">
      ...
      <q-card-section class="dialog-header">
        <div class="header-content">
          ...
          <q-btn flat round dense icon="arrow_back" class="close-btn" @click="goBack" />
        </div>
      </q-card-section>
    ...
    <OrganizeMatchesDialog :modelValue="organizeVisible" @update:modelValue="val => organizeVisible = val"
      :torneoId="displayTorneo?.id || torneoId" :initialTeams="organizeTeams" @generatedMatches="onGeneratedMatches"
      @started="onOrganizeStarted" />
  </q-page>
</template>
...
const torneoId = computed(() => route.params.id || route.query.torneoId)
const partidos = ref([])
const loading = ref(false)
const filterMode = ref('en_marcha')
const displayTorneo = ref(null)
...
onMounted(async () => {
  attachToIcon('.help-posiciones')

  const id = torneoId.value
  if (!id) {
    Notify.create({ type: 'negative', message: 'No se proporcionó un ID de torneo' })
    router.push('/torneos')
    return
  }
  ...
  try {
    ...
    // Si ya están todos finalizados, lanzar confetti
    await nextTick()
    ...
    Notify.create({ type: 'negative', message: 'Error al cargar el torneo' })
  } finally {
    loading.value = false
  }
})
...
async function onOrganizeStarted(evt) {
  try {
    const id = evt?.torneoId || displayTorneo.value?.id || torneoId.value
    if (id) {
      loading.value = true
      await loadPartidos(id)
...
async function confirmReprogram() {
  ...
  try {
    ...
    const id = displayTorneo.value?.id || torneoId.value
